...
class ParseBilibiliDanmaku {
  ...
  Future<List<DanmakuItem>> paseBilibiliDanmakuByXmlForXpath(String xmlPath,
      {String? xpath,
      String? attrName,
      String? splitChar,
    ...
    if (fromAssets) {
      document = await readXmlFromAssets(xmlPath);
    } else {
      document = readXmlFromPath(xmlPath);
    }
    if (document == null) {
      return [];
    }
    ...
    if (document.childElements.isNotEmpty) {
      ...
      for (XmlNode xmlNode in iterable) {
        ...
        DanmakuItem? danmakuItem =
            createDanmakuModel(readAttrTextList, xmlNode.innerText);
        if (danmakuItem != null) {
  ...
  // 逐级方式读取
  Future<List<DanmakuItem>> pasrseBilibiliDanmakuByXml(String xmlPath,
      {String? parentTag,
      String? contentTag,
      String? attrName,
      String? splitChar,
    ...
    for (XmlElement xmlElement in document.childElements) {
      ...
      if (xmlElement.localName == (parentTag ?? parentTagName)) {
        for (XmlElement element in xmlElement.childElements) {
          DanmakuItem? danmakuItem = getDanmakuItemByXmlElement(element,
              parentTag: parentTag,
        ...
        DanmakuItem? danmakuItem = getDanmakuItemByXmlElement(xmlElement,
            parentTag: parentTag,
            contentTag: contentTag,
            attrName: attrName,
            splitChar: splitChar);
  ...
  DanmakuItem? getDanmakuItemByXmlElement(
    XmlElement element, {
    String? parentTag,
    String? contentTag,
    String? attrName,
  ...
  // 生成弹幕内容
  DanmakuItem? createDanmakuModel(List<String> readAttrTextList, String text) {
